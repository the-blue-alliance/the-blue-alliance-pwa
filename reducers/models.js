...
const models = (state = Map(), action) => {
  switch (action.type) {
    ...
    case types.FETCH_EVENT_MATCHES_REQUEST:
      return state.setIn(['matchesStatus', 'collections', 'byEvent', `${action.eventKey}`], 'fetching')
    case types.FETCH_EVENT_MATCHES_SUCCESS:
      ...
      return state.setIn(['matchesStatus', 'collections', 'byEvent', `${action.eventKey}`], 'success')
    default:
      return state
  }
}
