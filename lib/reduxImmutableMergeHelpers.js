...
export const updateMulti = (
  ...
  let newModelsStatusByKey = Map();
  newModels.forEach(o => {
    newModelsByKey = newModelsByKey.set(o.get("key"), o);
    newModelsStatusByKey = newModelsStatusByKey.set(o.get("key"), "success");
  });

  // Merge new models into current models
  const byKeyPath = [modelType].concat(["byKey"]);
  const mergedModelsByKey = mergeDeepMulti(
    state.getIn(byKeyPath),
    newModelsByKey
  );
  const statusByKeyPath = [`${modelType}Status`].concat(["byKey"]);
  const mergedModelsStatusByKey = mergeDeepMulti(
    state.getIn(statusByKeyPath),
    newModelsStatusByKey
  );
  ...
  const collectionStatusPath = [`${modelType}Status`, "collections"].concat(
    partialPath
  );
  return state
    .setIn(byKeyPath, mergedModelsByKey)
    .setIn(collectionPath, mergedCollection)
    .setIn(statusByKeyPath, mergedModelsStatusByKey)
    .setIn(collectionStatusPath, "success");
};

const updateCollection = (state = Set(), newModelsByKey, merge) => {
  // Helper to merge collection into current collection
  if (state instanceof List) {
    // Might get a list due to SSR
    state = Set(state);
  }
  const newKeys = Set(newModelsByKey.keys());
  if (!merge) {
    // Whether to merge or overwrite the collection
