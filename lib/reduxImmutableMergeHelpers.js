...
export const updateMulti = (state, modelType, partialPath, newModels, merge = false) => {
  ...
  const mergedModelsStatusByKey = mergeDeepMulti(state.getIn(statusByKeyPath), newModelsStatusByKey)
  ...
  const collectionStatusPath = [`${modelType}Status`, 'collections'].concat(partialPath)
  return state.setIn(
    byKeyPath,
    mergedModelsByKey
  ).setIn(
