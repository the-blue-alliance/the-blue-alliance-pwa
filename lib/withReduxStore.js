...
function getOrCreateStore(initialState) {
  ...
  if (initialState) {
    ...
    Object.keys(MODEL_TYPES).forEach(key => {
      ...
      if (models) {
        initialState = initialState.setIn(modelPath, models.map(o => new MODEL_TYPES[key](o)))
      }
      ...
      if (collections) {
        collections.forEach((collectionEntries, collectionKey) => {
          initialState = convertCollections(initialState, collectionsPath, collectionEntries, collectionKey)
        })
      }
    })
  }
